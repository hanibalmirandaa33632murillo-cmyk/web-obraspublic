...
export default function DashboardSupervision() {
  ...
  if (loading) {
    return (
      <section style={{ background: "#020817", padding: "80px 40px", textAlign: "center" }}>
        <div
          style={{
            width: 48,
            height: 48,
    ...
    >
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        {stats && (
          ...
          >
            <Widget icon="🏗️" label="Obras Activas"    value={stats.obras_activas}    accent="#38bdf8" />
            <Widget icon="✅" label="Obras Terminadas" value={stats.obras_terminadas} accent="#4ade80" />
            <Widget icon="👥" label="Beneficiarios"    value={stats.total_beneficiarios.toLocaleString("es-MX")} accent="#a78bfa" />
            <Widget icon="💰" label="Presupuesto Ejercido" value={fmt(Number(stats.presupuesto_total_ejercido))} accent="#facc15" />
            <Widget icon="📋" label="Informes Este Mes" value={stats.informes_mes_actual} accent="#fb923c" />
            <Widget icon="📈" label="Avance Físico Promedio" value={`${stats.promedio_avance_fisico}%`} accent="#38bdf8" sub={`Presupuestal: ${stats.promedio_avance_presupuestario}%`} />
          </div>
        )}
        ...
        >
          ...
          {stats && (
            ...
            >
              ...
              <Barra label="Avance Físico"        valor={stats.promedio_avance_fisico}       color="#38bdf8" />
              <Barra label="Avance Presupuestal"  valor={stats.promedio_avance_presupuestario} color="#a78bfa" />
            </div>
          )}
          ...
          >
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr>
                  {["Obra", "Región", "Constructora", "Inicio", "Estado"].map((h) => (
                    <th
                      key={h}
                      style={{
                        textAlign: "left",
              ...
              <tbody>
                {obras.map((o) => {
                  ...
                  return (
                    <tr key={o.id_obra}>
                      <td style={{ padding: "10px 12px", color: "#e2e8f0", borderBottom: "1px solid #0f172a" }}>
                        {o.nombre_obra}
                      </td>
                      <td style={{ padding: "10px 12px", color: "#94a3b8", borderBottom: "1px solid #0f172a" }}>
                        {o.region?.comunidad ?? "—"}
                      </td>
                      <td style={{ padding: "10px 12px", color: "#94a3b8", borderBottom: "1px solid #0f172a" }}>
                        {o.constructora?.nombre_constructora ?? "H. Ayuntamiento"}
                      </td>
                      <td style={{ padding: "10px 12px", color: "#94a3b8", borderBottom: "1px solid #0f172a" }}>
        ...
        {informes.length > 0 && (
          ...
          >
            <p style={{ color: "#e2e8f0", fontWeight: 700, marginBottom: 20, fontSize: 15 }}>
              Informes Recientes
            </p>
            <div
              style={{
              ...
              {informes.slice(0, 6).map((inf) => (
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
                    ...
                    <span style={{ color: "#475569", fontSize: 12 }}>{inf.id_obra}</span>
                  </div>
                  <Barra label="Físico"       valor={inf.porcentaje_avance_fisico}       color="#38bdf8" />
                  <Barra label="Presupuestal" valor={inf.porcentaje_avance_presupuestario} color="#a78bfa" />
                  {inf.descripcion && (
